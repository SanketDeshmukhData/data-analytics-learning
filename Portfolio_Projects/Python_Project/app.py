...
tab_demo, tab_econ, tab_infra, tab_psych, tab_synth = st.tabs(
    ["👥 Demographics", "💰 Economics", "🔌 Infrastructure & Range", "🧠 Psychology & Awareness", "🎯 Best-Fit Segments"]
)
...
with tab_demo:
    ...
    with col1:
        fig_age = px.box(
            filtered_df, x="ev_adoption_likelihood", y="age",
            category_orders={"ev_adoption_likelihood": ADOPTION_ORDER},
            color="ev_adoption_likelihood", color_discrete_map=COLOR_MAP,
    ...
    with col2:
        fig_income = px.box(
            filtered_df, x="ev_adoption_likelihood", y="annual_income",
            category_orders={"ev_adoption_likelihood": ADOPTION_ORDER},
            color="ev_adoption_likelihood", color_discrete_map=COLOR_MAP,
    ...
    city_props = (
        pd.crosstab(filtered_df["city_type"], filtered_df["ev_adoption_likelihood"], normalize="index")[ADOPTION_ORDER]
        * 100
    ).reset_index().melt(id_vars="city_type", var_name="ev_adoption_likelihood", value_name="pct")

    fig_city = px.bar(
        city_props, x="city_type", y="pct", color="ev_adoption_likelihood",
        category_orders={"ev_adoption_likelihood": ADOPTION_ORDER},
        color_discrete_map=COLOR_MAP, title="Adoption Likelihood % by City Type",
...
with tab_econ:
    ...
    cost_by_group = (
        ...
        ].mean().reindex(ADOPTION_ORDER).reset_index()
        .melt(id_vars="ev_adoption_likelihood", var_name="cost_type", value_name="amount")
    )

    fig_cost = px.bar(
        cost_by_group, x="ev_adoption_likelihood", y="amount", color="cost_type",
        barmode="group", category_orders={"ev_adoption_likelihood": ADOPTION_ORDER},
        title="Avg Monthly Fuel vs. Charging Cost, by Adoption Likelihood",
        labels={"amount": "₹ per month", "ev_adoption_likelihood": "Adoption likelihood"},
    )
    ...
    st.metric("Avg. potential monthly savings by switching to EV", f"₹{avg_savings:,.0f}")
...
with tab_infra:
    ...
    with col1:
        fig_dist = px.box(
            filtered_df, x="ev_adoption_likelihood", y="nearest_charging_station_km",
            category_orders={"ev_adoption_likelihood": ADOPTION_ORDER},
            color="ev_adoption_likelihood", color_discrete_map=COLOR_MAP,
    ...
    with col2:
        fig_anx = px.box(
            filtered_df, x="ev_adoption_likelihood", y="range_anxiety_score",
            category_orders={"ev_adoption_likelihood": ADOPTION_ORDER},
            color="ev_adoption_likelihood", color_discrete_map=COLOR_MAP,
    ...
    home_props = (
        pd.crosstab(filtered_df["home_charging_available"], filtered_df["ev_adoption_likelihood"], normalize="index")[ADOPTION_ORDER]
        * 100
    ).reset_index().melt(id_vars="home_charging_available", var_name="ev_adoption_likelihood", value_name="pct")
    home_props["home_charging_available"] = home_props["home_charging_available"].map({0: "No", 1: "Yes"})

    fig_home = px.bar(
        home_props, x="home_charging_available", y="pct", color="ev_adoption_likelihood",
        category_orders={"ev_adoption_likelihood": ADOPTION_ORDER},
        color_discrete_map=COLOR_MAP, title="Adoption Likelihood % by Home Charging Availability",
        labels={"pct": "% of customers", "home_charging_available": "Home charging available"},
    )
...
with tab_psych:
    ...
    correlations = filtered_df[score_cols].corrwith(target_numeric).sort_values().reset_index()
    correlations.columns = ["factor", "correlation"]

    fig_corr = px.bar(
        correlations, x="correlation", y="factor", orientation="h",
    ...
    exp_props = (
        pd.crosstab(filtered_df["previous_ev_experience"], filtered_df["ev_adoption_likelihood"], normalize="index")[ADOPTION_ORDER]
        * 100
    ).reset_index().melt(id_vars="previous_ev_experience", var_name="ev_adoption_likelihood", value_name="pct")
    exp_props["previous_ev_experience"] = exp_props["previous_ev_experience"].map({0: "No prior experience", 1: "Prior experience"})

    fig_exp = px.bar(
        exp_props, x="previous_ev_experience", y="pct", color="ev_adoption_likelihood",
        category_orders={"ev_adoption_likelihood": ADOPTION_ORDER},
        color_discrete_map=COLOR_MAP, title="Adoption Likelihood % by Previous EV Experience",
...
with tab_synth:
    ...
    st.caption("Segments below require at least 200 customers, to avoid ranking on small, unreliable groups.")

    segment_summary = (
        filtered_df.groupby(["city_type", "current_vehicle_type", "home_charging_available"], observed=True)
        .agg(
    ...
    if segment_summary.empty:
        st.warning("No segment has 200+ customers under the current filters — widen your filter selection.")
    else:
        top_segments = segment_summary.sort_values("pct_high", ascending=False).head(10)
        top_segments["home_charging_available"] = top_segments["home_charging_available"].map({0: "No home charging", 1: "Home charging"})
        top_segments["segment_label"] = (
            top_segments["city_type"] + " / " + top_segments["current_vehicle_type"] + " / " + top_segments["home_charging_available"]
        )

        fig_seg = px.bar(
            top_segments.sort_values("pct_high"), x="pct_high", y="segment_label", orientation="h",
            title="Top Segments by % High Adoption Likelihood (min. 200 customers)",
            labels={"pct_high": "% High adoption likelihood", "segment_label": "Segment"},
            color="pct_high", color_continuous_scale="Greens",
            hover_data=["segment_size"],
        )
        st.plotly_chart(fig_seg, use_container_width=True)

        st.dataframe(
            top_segments[["city_type", "current_vehicle_type", "home_charging_available", "segment_size", "pct_high"]]
            .rename(columns={"pct_high": "% High adoption", "segment_size": "Segment size"})
